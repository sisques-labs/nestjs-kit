 ...
 * never become labels.
 *
 * Note: this file imports `@nestjs/graphql` unconditionally (for
 * `GqlExecutionContext`), so `@nestjs/graphql` is a real runtime dependency of
 * `@sisques-labs/nestjs-kit/metrics` even for REST-only apps — not lazily
 * loaded. Both current consumers already depend on it for their GraphQL
 * transport.
 */
@Injectable()
