 ...
 *
 * - Topic: `${topicPrefix}.${module}` (e.g. `my-service.plants`).
 ...
 *
 * Reads its config from `ConfigService.getOrThrow<IKafkaConfig>('kafka')` — the
 * consuming app registers a config factory under that namespace (env-var reading
 * and the app-specific default client id/topic prefix are the app's concern).
 */
@Injectable()
