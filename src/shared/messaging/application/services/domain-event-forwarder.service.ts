...
import {
  EventRoutingService,
  deriveAction,
} from '../../domain/routing/event-routing.service';
import { IOutboundEvent } from '../../domain/interfaces/outbound-event.interface';
import {
  EVENT_PUBLISHER,
  IEventPublisher,
} from '../../domain/ports/event-publisher.port';
import { IKafkaConfig } from '../../infrastructure/kafka/kafka-config.interface';
  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  constructor(
    ...
    private readonly eventRouting: EventRoutingService,
  ...
  private async forward(event: IEvent): Promise<void> {
    ...
    if (
      ...
      this.logger.warn(
        `No module mapping for "${event.aggregateRootType}" — routing "${event.eventType}" to the "unmapped" topic. Add it to the app's aggregate-module map.`,
      );
    }
  ...
  private toOutboundEvent(event: DomainEventLike): IOutboundEvent {
    return {
      module: this.eventRouting.resolveModule(event.aggregateRootType).module,
